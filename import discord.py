...
class Client(commands.Bot):
    ...
    async def on_message(self, message):
        ...
        if message.content.startswith("!player"):
            player_name = message.content.split("!player ", 1)[1]
            stats = get_player_stats(player_name)
            if stats:
                await message.channel.send(embed=stats)
            else:
                await message.channel.send("Couldn't find stats for the specified player.")


        if message.content.startswith("!stats"):
            team_name = message.content.split("!stats ", 1)[1]  # Extract the team name
            stats2 = get_team_stats(team_name)
            if stats2:
                await message.channel.send(embed=stats2)
            else:
                await message.channel.send("Couldn't find stats for the specified team.")
...
def get_player_stats(player_name):
    ...
    headers = {
        "x-rapidapi-key": "6bde8738b937b02353bdf65f04ac1caf",
        "x-rapidapi-host": "v3.football.api-sports.io"
    }
    params = {"search": player_name, "season": 2023, "league": 39}
    ...
    if data.get("response"):
        ...
        **Games Played:** {statistics['games']['appearences']}
        **Minutes Played:** {statistics['games']['minutes']}
        **Yellow Cards:** {statistics['cards']['yellow']}
        **Red Cards:** {statistics['cards']['red']}
        ***Rating:*** {statistics['games']['rating']}


        """
        embed = discord.Embed(title=f"{player['firstname']} {player['lastname']}",
                              description=description, color=0x1DB954)
...
def get_team_stats(team_name):
    ...
    if data.get("response"):
        ...
        **Venue:** {team['venue']['name']}, {team['venue']['city']}
        """
        embed = discord.Embed(title=f"{team['team']['name']} Stats", description=stats2, color=0x1DB954)
        embed.set_thumbnail(url=team['team']['logo'])
        return embed
    else:
        return None
